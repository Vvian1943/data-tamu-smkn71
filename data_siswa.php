...
<body>
  <div class="wrapper">
    ...
    <div class="container">
      <h2><i class="fas fa-user-graduate"></i> Data Siswa SMKN 71 Jakarta</h2>

      <div class="actions">
        <a href="tambah.php" class="btn btn-primary"><i class="fas fa-plus"></i> Tambah Data</a>
        <a href="cetak_siswa.php" class="btn btn-gray" target="_blank"><i class="fas fa-print"></i> Cetak Data</a>
      </div>

      <div class="search-bar">
        <form method="GET">
          <input type="text" name="cari" placeholder="Cari Nama, Kelas, Jurusan..."
                 value="<?php echo isset($_GET['cari']) ? htmlspecialchars($_GET['cari']) : ''; ?>">
          <button type="submit" class="btn btn-primary"><i class="fas fa-search"></i> Cari</button>
          <a href="data_siswa.php" class="btn btn-success"><i class="fas fa-sync-alt"></i> Refresh</a>
        </form>
      </div>
          ...
          if ($result && mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
              echo "<tr>
                      ...
                      <td>{$row['Jurusan']}</td>
                      <td>
                        <a href='edit.php?edit={$row['id']}' class='btn btn-success'><i class='fas fa-edit'></i> Edit</a>
                        <a href='delete.php?hapus={$row['id']}' class='btn btn-danger' onclick='return confirm(\"Yakin ingin menghapus?\")'><i class='fas fa-trash'></i> Hapus</a>
                      </td>
                    </tr>";
            }
          } else {
            echo "<tr><td colspan='5' style='text-align:center; padding:40px;'><i class='fas fa-database'></i> Tidak ada data Siswa.</td></tr>";
          }
          ?>
        </tbody>
      </table>
